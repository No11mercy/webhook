...
def execute_trade(signal, symbol):
    try:
        ...
        if used_balance > 0:
            ...
            # Если нет позиции, зайдите на 10% от доступного баланса
...
def webhook():
    # Логируем тип контента для диагностики
    ...
    try:
        data = request.get_json()  # Используйте get_json() для безопасного получения JSON данных
        if not data:
