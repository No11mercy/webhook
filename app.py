...
def webhook():
    if request.method == 'POST':
        try:
            # Выводим заголовки запроса и тело для отладки
            logging.debug("Заголовки запроса: %s", request.headers)
            logging.debug("Тело запроса: %s", request.data)
            ...
            if status_code == 200:
                ...
                logging.error("Ошибка отправки сообщения в Telegram: %s", response_text)
